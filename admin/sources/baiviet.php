...
function get_mans(){
	...
	if($_REQUEST['banchay']!='')
	{
		...
		$sql_sp = "SELECT id,banchay,hot FROM table_baiviet where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$time=time();
	...
	{
		$id_up = $_REQUEST['hot'];
		$sql_sp = "SELECT id,hot FROM table_baiviet where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$time=time();
		$hienthi=$cats[0]['hot'];
		if($hienthi==0)
		{
			$sqlUPDATE_ORDER = "UPDATE table_baiviet SET hot =1 WHERE  id = ".$id_up."";
			$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
		}
		else
		{
			$sqlUPDATE_ORDER = "UPDATE table_baiviet SET hot =0  WHERE  id = ".$id_up."";
			$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
		}	
	}
	#-------------------------------------------------------------------------------
	...
	$sql = "select ten_vi,id,stt,hienthi,id_list,id_cat,noibat,id_item,id_sub,banchay,hot,more_vi from $where $limit";
	$d->query($sql);
...
function save_man(){
	...
	$file_upfile=images_name($_FILES['upfile']['name']);
	...
	if($id){
		...
		if($file_d = upload_image("upfile", 'rar|zip|xls|xlsx|pdf|doc|docx|ppt|pptx|exe', _upload_baiviet,$file_upfile)){
			$data['upfile'] = $file_d;	
			$d->setTable('baiviet');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_baiviet.$row['upfile']);	
			}
		}
		...
		$data['mota_cn'] = addslashes($_POST['mota_cn']);
		$data['hotline'] = $_POST['hotline'];
		$data['more'] = $_POST['more'];
		$data['more_vi'] = addslashes($_POST['more_vi']);
		$data['more_en'] = addslashes($_POST['more_en']);
		$data['more_cn'] = addslashes($_POST['more_cn']);
		$data['noidung_vi'] = addslashes($_POST['noidung_vi']);
		$data['noidung_cn'] = addslashes($_POST['noidung_cn']);
