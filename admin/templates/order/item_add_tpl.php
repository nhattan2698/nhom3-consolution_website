			...
			<tbody>
				...
				for($i=0,$count_donhang=count($result_ctdonhang);$i<$count_donhang;$i++){	
					...
					<tr id="productct<?=$result_ctdonhang[$i]['id']?>">
						...
						<td><?=$pname?>
							<?php 
							$array_size= get_size($result_ctdonhang[$i]['size']);
							$array_color= get_color($result_ctdonhang[$i]['mausac']);	
							?>
							<p>Size : <?=$array_size['ten_'.$lang];?></p>
							<p>Màu sắc : <?=$array_color['ten_'.$lang];?></p>
						</td>
