...
function get_video_youtube($width=2,$height=1,$select=true,$auto=false ){
	...
	//script
	$showtext.='<script>$(function(){var o=$("#box_video").data("id"),d=$("#box_video").data("width"),a=$("#box_video").data("height"); $("#box_video").load("ajax/load_video.php?id="+o+"&width="+d+"&height="+a);
	    $("#video_lienquan").change(function(){var i=$(this).val(); $("#box_video").load("ajax/load_video.php?id="+i); });});</script>';
	/*copright by bossini - M.Tân - tan1993.CIPMEDIA@gmail.com */
	echo $showtext;
}

function get_video_youtube2($width=2,$height=1,$select=true,$auto=false ){
	global $d;
	$ratio = round($height/$width*100,2);
	//get database
	$sql_video = "select ten_vi as ten,photo,links from #_video where type='meovat' and hienthi=1  order by stt,id desc";
	$d->query($sql_video);
	$video = $d->result_array();
	$chuoi=explode("=",$video[0]['links']);
	//showtext div load
	$showtext .= '<div style="padding-bottom: '.$ratio.'%;position:relative;width:100%;height: 0;" id="box2_video" data-id="'.$chuoi[1].'" data-width="'.$width.'" data-height="'.$height.'" data-auto="'.$auto.'" ></div>';
	if($select){
		$showtext .= '<div class="select_video"><select width="100%" id="video2_lienquan">';
		$showtext .='<option value="">Video khác</option>';
		for ($i=0,$count=count($video); $i < $count; $i++) { 
			$chuoi=explode("=",$video[$i]['links']);
			$showtext .='<option value="'.$chuoi[1].'">'.$video[$i]['ten'].'</option>';
...
function get_video_youtube_2($width=2,$height=1,$select=true,$auto=false ){
	...
	$showtext .= '<div style="padding-bottom: '.$ratio.'%;position:relative;width:100%;height: 0;" id="box_video" data-id="'.$chuoi[1].'" data-width="'.$width.'" data-height="'.$height.'" data-auto="'.$auto.'" ></div>';
	if($select){
		$showtext .= '<div class="select_video owl-carousel_video">';
		for ($i=0,$count=count($video); $i < $count; $i++) { 
			$chuoi=explode("=",$video[$i]['links']);
...
function get_social($url,$type="share",$width="100%",$height="auto"){
	switch ($type) {
		...
		case 'fanpage':
			...
			$showtext = '<div id="fanpage" data-url="'.$url.'" data-tabs="timeline" data-width="'.$width.'" data-height="'.$height.'" ></div>';
			$showtext.='';
			break;
		case 'comment':
			if($url=='') $url= getCurrentPageURL();
...
function get_mapapi($width=2,$height=1,$coor='',$zoom=16){
	...
	$showtext .='<script>$(function(){
		$("#map_canvas").load("ajax/load_map.php?coor="+$("#map_canvas").data("coor")+"&zoom="+$("#map_canvas").data("zoom")); });</script>';
	echo $showtext;
}
//get url now
?>