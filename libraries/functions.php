...
function compress($buffer){
 ...
 function Organization(){
		...
		{ "@context" : "http://schema.org",
		  ...
		  "logo":"'._SITEURLs_._upload_hinhanh_l.$Logo.'",
		  "contactPoint" : [
		    {
		      "@type" : "ContactPoint",
		      "telephone" : "+84 '.$row_setting['dienthoai'].'",
...
function getSQLs($select,$table,$where, $limit='',$oderby='stt,id'){
	$where.= ($where=='' || empty($where)) ? ' hienthi=1 ' : ' and hienthi=1 ';
	global $d;
	$d->reset();
	$sql = 'select '.$select.' from #_'.$table.' where '.$where.' order by '.$oderby;
...
function getSQL($select,$table,$where,$oderby=''){
	$where.= ($where=='' || empty($where)) ? ' hienthi=1 ' : ' and hienthi=1 ';
	global $d;
	$d->reset();
	$sql = 'select '.$select.' from #_'.$table.' where '.$where;
...
function adminmenu($ten,$com,$act,$type){
	global $d;
...
function sitemenu($comset='./',$text='Home', $level=0, $table='', $type='',$parent = 1){
	global $d, $com, $lang;
	if ($parent==1) {$rs_string = "<li class='".(($com==$comset || ($comset=='./' && $com=='')) ? 'active' : '')."'><a href='$comset'>$text</a>"; }
	//Bài viết
	if ($level==-1) {
		$d->reset();
		$sql = "select ten_$lang as ten, id, tenkhongdau as url from #_".$table." where hienthi=1 and type='$type'  order by stt,id desc limit 15";
		$d->query($sql);
		$Content = $d->result_array();
		if (!empty($Content)) {
			$rs_string .= '<ul class="">';
			foreach ($Content as $kc => $vi) {
				$ten = $vi['ten'];
				$url = $vi['url'];
				$rs_string .= "<li><a href='$url'>$ten</a>";
				$rs_string .= "</li>";
			}
			$rs_string .= '</ul>';
		}
	...
	if ($level>=1) {
		$d->reset();
		$sql = "select ten_$lang as ten, id, tenkhongdau as url from #_".$table."_list where hienthi=1 and type='$type'  order by stt,id desc";
		$d->query($sql);
		$List = $d->result_array();
		if (!empty($List)) {
			if ($parent!=0) {$rs_string .= '<ul class="">'; }
			foreach ($List as $kl => $vl) {
				...
				if ($level>=2) {
					$d->reset();
					$sql = "select ten_$lang as ten, id, tenkhongdau as url from #_".$table."_cat where hienthi=1 and type='$type' and id_list='".$vl['id']."'  order by stt,id desc";
					$d->query($sql);
					$Cat = $d->result_array();
					if (!empty($Cat)) {
						$rs_string .= '<ul class="">';
				 ...
				 //  Cấp 3
							if ($level>=3) {
								$d->reset();
								$sql = "select ten_$lang as ten, id, tenkhongdau as url from #_".$table."_item where hienthi=1 and type='$type'  and id_cat='".$vc['id']."' and id_list='".$vl['id']."' order by stt,id desc";
								$d->query($sql);
								$Item = $d->result_array();
								if (!empty($Item)) {
									$rs_string .= '<ul class="">';
			...
			if ($parent!=0) { $rs_string .= '</ul>';}
		}
	}
	if ($parent==1) {$rs_string .= '</li>'; }
	return $rs_string;
}


function encrypt_password($str){
	// echo _encrypt_salt.$str._encrypt_pepper;
...
class paging_ajax
{
    ...
    public function GetResult()
    {
        ...
        return mysqli_query($this->text_sql." LIMIT $start, $this->per_page");
    }
...
function getsub_imgpro($pid = false){
	global $d;
	$d->reset();
	$sql = "select photo from #_product_photo where type='product' and id_product='".$pid."' order by stt,id desc limit 1";
	$d->query($sql);
	$result_PhotoProduct = $d->fetch_array();
	return $result_PhotoProduct['photo'];
}

function getaddress_name($id,$type){
	global $d,$lang;
	$sql = "select ten from #_place_".$type." where id='".$id."' limit 1";
	$d->query($sql);
	$return = $d->fetch_array();
	return $return['ten'];
}
function numforvn( $num = false ) {
	$str = '';
	$num  = trim($num);    
	$arr = str_split($num);
	$count = count( $arr );    
...
function numforvn_long( $num = false ) {
	$str = '';
	$num  = trim($num);    
	$arr = str_split($num);
	$count = count( $arr );    
...
function getimgyoutube_max($urlYoutube){
	$idYoutube=end(explode('v=',$urlYoutube));
	return $ImgYoutube="http://img.youtube.com/vi/".$idYoutube."/maxresdefault.jpg";
}
function countProduct($name_danhmuc,$value_danhmuc){
	global $d;
	$d->reset();
	$sql = "select * from #_products";
	$d->query($sql);
	$reuslr_products = $d->fetch_array();
}
function check_new($id){
	global $d;
	$d->reset();
	$sql = "select * from #_product where id ='".$id."' and  spmoi >0";
	$d->query($sql);
	$reuslr_products = $d->result_array();
	if(count($reuslr_products)>0){
		echo '<div class="icon_news"></div>';
	}
}
...
function getTigia($amount=1, $from='USD', $to='VNĐ'){
	$data = file_get_contents("https://www.google.com/finance/converter?a=$amount&from=$from&to=$to");
	preg_match("/<span class=bld>(.*)<\/span>/",$data, $converted);
	$converted = preg_replace("/[^0-9.]/", "", $converted[1]);
	return round($converted, 3);
	// echo getTigia("1", "USD", "VND");
}
//////////////////////////////================ end =========
...
function show_tags($tags,$lass = "tags"){
	...
	for ($i=0,$count=count($arr); $i < $count; $i++) { 
		...
		$d->query($sql);
		$row = $d->fetch_array();
		echo '<li><a href="'.$row["tenkhongdau"].'" class="'.$lass.'"><span></span>'.$row["ten_vi"].'</a></li>';
	}
}
...
function getproduct_fromid($id=0){
	global $d,$lang;
	$d->reset();
	$sql = "select ten_$lang,id,tenkhongdau,photo,thumb,mota_$lang,giacu,giaban from #_product where type='product' and id='".$id."'";
	$d->query($sql);
	$result_product = $d->fetch_array();
