...
function save_banner(){
	...
	$file_name1=images_name($_FILES['file_en']['name']).'_en';
	$file_name2=images_name($_FILES['file_cn']['name']).'_cn';
	...
	if($id){ 
		...
		$d->setWhere('type',$_GET['type']);
		if($d->update($data))
			redirect("index.php?com=setphoto&act=capnhat&type=".$_GET['type']."");
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=setphoto&act=capnhat&type=".$_GET['type']."");
	}else{ // them moi
		...
		if($photo = upload_image("file_vi", 'swf|jpg|gif|png', _upload_hinhanh,$file_name1)){
			$data['photo_vi'] = $photo;
		...
		redirect("index.php?com=setphoto&act=capnhat&type=".$_GET['type']."");
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=setphoto&act=capnhat&type=".$_GET['type']."");
